// User argument is from godfather (https://github.com/Stitch07/godfather) Copyright 2020 Stitch07, used under the AGPL-3.0 License
...
export default class extends Argument<User> {
	public async run(parameter: string, { message }: ArgumentContext): AsyncArgumentResult<User> {
		...
		return this.error({ parameter, identifier: 'InvalidUserError', message: 'The argument did not resolve to a valid user.' });
	}
}
